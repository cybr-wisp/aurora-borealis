...
class OperatorSimulation:
    ...
    def reset(self) -> None:
        ...
        self.operating_center_xy = np.mean(
            np.asarray(
                [cfg.position_enu_m[:2] for cfg in self.configs],
                dtype=float,
            ),
            axis=0,
        )

        self.operating_soft_radius_m = OPERATIONAL_SOFT_RADIUS_M
        self.operating_hard_radius_m = OPERATIONAL_HARD_RADIUS_M
        self.spawn_safe_radius_m = SPAWN_SAFE_RADIUS_M
    ...
    def _create_target(self) -> RuntimeTarget:
        ...
        if not self.targets:
            ...
            active_positions = np.stack(
                [target.position for target in self.targets.values()]
            )

            center = np.mean(active_positions, axis=0)

            angle = (target_id - 2) * 2.399963229728653

            radius = 1100.0 + 350.0 * max(0, index - 1)
            ...
            velocity = primary.velocity + np.array(
                [
                    -5.0 - 2.0 * index,
                    8.0 + 3.0 * index,
                    0.1 * index,
        ...
        spawn_offset = position[:2] - self.operating_center_xy

        spawn_radius = float(np.linalg.norm(spawn_offset))

        if spawn_radius > self.spawn_safe_radius_m:
            position = position.copy()

            position[:2] = (
                self.operating_center_xy
                + (spawn_offset / spawn_radius) * self.spawn_safe_radius_m
            )
    ...
    def command(self, payload: dict) -> None:
        ...
        if action == "kill_sensor" and sid in self.enabled:
            ...
            self.health[sid] = "DEGRADED" if self.r_est[sid] >= 2.0 else "NOMINAL"

            self.message = f"sensor {sid} restored"

        elif action == "inject_bias" and sid in self.bias_m:
            self.bias_m[sid] = 25.0 if self.bias_m[sid] == 0.0 else 0.0

            self.message = (
                f"sensor {sid} range bias {'on' if self.bias_m[sid] else 'off'}"
            )

        elif action == "add_noise" and sid in self.noise_scale:
            self.noise_scale[sid] = 4.0 if self.noise_scale[sid] == 1.0 else 1.0
            ...
            if target is None:
                self.message = f"cannot maneuver unknown TRK-{target_id:03d}"
            else:
                target.maneuver_until_sec = self.t + 5.0

                self.message = f"maneuver injected into TRK-{target.target_id:03d}"

        elif action == "add_target":
            target = self._create_target()

            self.message = f"added independent target TRK-{target.target_id:03d}"

        elif action == "toggle_filter":
            self.filter_name = "UKF" if self.filter_name == "EKF" else "EKF"
    ...
    def _truth_state(self, target: RuntimeTarget) -> TargetState:
        ...
        if self.t < target.maneuver_until_sec:
            direction = -1.0 if target.target_id % 2 == 0 else 1.0
            ...
            target.velocity += acceleration * DT
        ...
        horizontal_offset = target.position[:2] - self.operating_center_xy

        horizontal_radius = float(np.linalg.norm(horizontal_offset))

        if horizontal_radius > self.operating_soft_radius_m:
            radial_hat = horizontal_offset / horizontal_radius
            ...
            excess_distance = horizontal_radius - self.operating_soft_radius_m

            speed_error = max(
                0.0,
                radial_speed + BOUNDARY_DESIRED_INWARD_MPS,
            )

            inward_accel = min(
                BOUNDARY_MAX_ACCEL_MPS2,
                (BOUNDARY_SPEED_GAIN * speed_error)
                + (BOUNDARY_DISTANCE_GAIN * excess_distance),
            )

            target.velocity[:2] -= radial_hat * inward_accel * DT

        # Normal continuous integration.
        next_position = target.position + target.velocity * DT
        ...
        # velocity.
        next_offset = next_position[:2] - self.operating_center_xy

        next_radius = float(np.linalg.norm(next_offset))

        if next_radius > self.operating_hard_radius_m:
            normal = next_offset / next_radius
            ...
            if outward_speed > 0.0:
                target.velocity[:2] -= 1.8 * outward_speed * normal

            next_position[:2] = (
                self.operating_center_xy + normal * self.operating_hard_radius_m
            )
    ...
    def _adapt_sensor_noise(
        ...
        self.r_est[sensor_id] = 0.95 * self.r_est[sensor_id] + 0.05 * ratio
    ...
    def step(self) -> dict:
        ...
        for radar, cfg in zip(self.radars, self.configs):
            ...
            for target_id, truth in truths.items():
                ...
                for observation in sensor_observations:
                    ...
                    raw_position = sensor_position + spherical_to_cartesian(
                        float(z[0]),
                        float(z[1]),
                        float(z[2]),
                    )
                    ...
                    innovation[1:] = (innovation[1:] + math.pi) % (
                        2.0 * math.pi
                    ) - math.pi

                    predicted_measurement_cov = (
                        measurement_h @ target.filter.p @ measurement_h.T
                    )
        ...
        for target_id, target in sorted(self.targets.items()):
            ...
            try:
                nees = float(error @ np.linalg.solve(target.filter.p, error))
        ...
        for cfg in self.configs:
            ...
            packet_loss = (
                ...
                else 100.0 * (1.0 - self.received[sid] / self.generated[sid])
            )
        ...
        total_loss = (
            ...
            else 100.0 * (1.0 - total_received / total_generated)
        )

        live_update_ms = (time.perf_counter() - started) * 1000.0
